...
class GitCommit:
    ...
    def update_s3_data(cls):
        ...
        print(f"Uploading updated commit list [{commits}]")
    ...
    def dump(cls, commits):
        commits_ = []
        for commit in commits:
            commits_.append(json.dumps(dataclasses.asdict(commit)))
    ...
    def push_to_s3(cls, commits):
        print("INFO: push commits data to s3, commits num [{len(commits)}]")
        cls.dump(commits)
        local_path = Path(cls.file_name())
        file_name = local_path.name
        env = _Environment.get()
